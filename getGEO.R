...
quick.image = function(spdata, lon.map, lat.map, type=NULL, same=FALSE, zlim=NULL, col=tim.colors(32),
                       nlevel=32, title='', width=6.5, height=4, mai=c(0.5, 0.5, 0.5, 0.5), mgp=c(1.4, 0.5, 0), tcl=-0.3, ps=12, 
                       legend.mar=4, legend.width=1.2, scaleF=NULL, labelsIN=NULL, addKT=NULL) {
  require(fields)
  require(maps)
  ...
  if (length(dim(spdata)) == 2) {
    ...
    #par(mfrow=mfrow, mai=mai, mgp=mgp, tcl=tcl, ps=ps, cex=1)
    if (is.null(type)) {
      ...
      tmp =  c( abs(min(na.omit(as.vector(spdata)))),max(na.omit(as.vector(spdata))))
      zlim = c(-max(na.omit(as.vector(tmp)))/2, max(na.omit(as.vector(tmp)))/2)
      print(zlim)
      col = rwb.colors(nlevel)
    }
    else if (type == "def") {
    ...
    image.plot(lon.map, lat.map, spdata, zlim=zlim, xlab="", ylab="", axis.args=list(mgp=c(0,0.3,0), tcl=-0.1), legend.mar=legend.mar,legend.lab=c("Nsamples"), legend.width=legend.width, col=col, nlevel=nlevel)
    if (length(addKT) > 0){map("world", add=1)}
    text(128, 4,labels = labelsIN,cex=2)
    print(labelsIN)

  }
  
  else {
    num.plot = dim(spdata)[1]
    if (num.plot == 2)
      mfrow = c(1,2)
    else if (num.plot == 3)
      mfrow = c(1,3)
    ...
    for (k in 1:num.plot) {
      if (is.null(type[k])) {
       ...
       zlim=c(-100,100) 
        ...
        tmp =  c( abs(min(na.omit(as.vector(spdata)))),max(na.omit(as.vector(spdata))))
        zlim = c(-max(na.omit(as.vector(tmp))), max(na.omit(as.vector(tmp))))
        print(zlim)
        col = rwb.colors(nlevel)
      }
      else if (type[k] == "krt2") {
      ...
      if (length(scaleF) != num.plot){
        image.plot(lon.map, lat.map, spdata[k,,], zlim=zlim, xlab="", ylab="", 
                 axis.args=list(mgp=c(0,0.3,0), tcl=-0.1), legend.mar=legend.mar, legend.width=legend.width, col=col, nlevel=nlevel)
      } else {  image.plot(lon.map, lat.map, spdata[k,,]*scaleF[k], zlim=zlim, xlab="", ylab="", 
                           axis.args=list(mgp=c(0,0.3,0), tcl=-0.1), legend.mar=legend.mar, legend.width=legend.width, col=col, nlevel=nlevel)}
      text(126, 39,labels = labelsIN[k],cex=2)
      if (length(addKT) > 0){map("world", add=1)}
      #points(olon, olat, col='red')
      require(ncdf4)
      files3b=list.files("/Volumes/KatiesFiles/GEOSChem/12.9.3_025x03125/","*Species*",full.names = TRUE)
      tt = nc_open(files3b[1]);  gclon=ncvar_get(tt,varid="lon") ; gclat=ncvar_get(tt,varid="lat");  nc_close(tt)
      ind = which(abs(gclon - olon) == min(abs(gclon - olon))) 
      ind2 = which(abs(gclat - olat) == min(abs(gclat - olat)))
      gcolon =gclon[ind] ; gcolat = gclat[ind2]
      bot = gcolat - 0.25/2 ; top = gcolat + 0.25/2
      left = gcolon - 0.3125/2 ; right = gcolon + 0.3125/2
      segments(left,bot, x1 = right, y1 = bot)
      segments(left,bot, x1 = left, y1 = top)
      segments(right,bot, x1 = right, y1 = top)
      segments(left, top, x1 = right, y1 = top)
...
plot.field = function(spdata, lon.map, lat.map, type=NULL, same=FALSE, zlim=NULL, image.only=FALSE, legend.only=FALSE,
                      horizontal=FALSE, col=NULL, nlevel=32, mai=c(0.2, 0.2, 0.1, 0.1), mgp=c(1.4, 0.5, 0), tcl=-0.3, ps=12,
                      legend.mar=3, legend.width=1.2, xaxt="n", yaxt="n", Pacific.centric=FALSE, country=NULL, 
                      custom.breaks=NULL, map.xlim=NULL, map.ylim=NULL) {
  ...
  if (is.null(custom.breaks)) {
    ...
    if (image.only) {
      ...
      image.plot(lon.map, lat.map, spdata, zlim=zlim, xlab='', ylab='', axis.args=list(mgp=mgp, tcl=tcl), 
                 legend.lab=c("Nsamples"),legend.only=legend.only, horizontal=horizontal, legend.mar=legend.mar, legend.width=legend.width, col=col, nlevel=nlevel, xaxt=xaxt, yaxt=yaxt)
    }
    ...
    if (image.only) {
      ...
      image.plot(lon.map, lat.map, spdata, zlim=zlim, xlab='', ylab='', axis.args=list(mgp=mgp, tcl=tcl), legend.only=legend.only, legend.lab=c("nsamples"),
                 horizontal=horizontal, legend.mar=legend.mar, legend.width=legend.width, col=col, nlevel=nlevel, xaxt=xaxt, yaxt=yaxt, breaks=zval.breaks, lab.breaks=custom.breaks)
    }
...
plot.site = function(spdata, latlon, xlim=NULL, ylim=NULL, pch=19, type=NULL,cex=cex,
                     zlim=NULL, col=NULL, nlevel=32, mai=c(0.5, 0.4, 0.2, 0.2), mgp=c(1.4, 0.5, 0), 
                     tcl=-0.3, ps=12, legend.mar=3, legend.width=1.2, xaxt="n", yaxt="n", Pacific.centric=FALSE, add=FALSE) {
  ...
  if (is.null(xlim)) xlim = range(latlon[,2], na.rm=TRUE)
  if (is.null(ylim)) ylim = range(latlon[,1], na.rm=TRUE)
  ...
  if (!add) {
    par(mai=mai, mgp=mgp, tcl=tcl, ps=ps)
    image.plot(seq(xlim[1], xlim[2], length=10), seq(ylim[1], ylim[2], length=10), matrix(zlim[1]-1, nrow=10, ncol=10), 
               zlim=zlim, 
                axis.args=list(mgp=c(0,0.5,0), tcl=-0.2), legend.mar=legend.mar, #xlab="",ylab="",
               legend.width=legend.width, col=col, nlevel=nlevel, xaxt=xaxt, yaxt=yaxt)
  }
...
make.land.mask = function(lon, lat) {
  ...
  # It makes use of another function "CountryData2LonLat" in "get_geo.R" (see above).
  # It only works for a longitude range of -180 deg to 180 deg.
  
  # This function requires installation of two external packages:
  library(rworldmap); library(raster)
  
  # All possible country codes:
  # "countryRegions" from "rworldmap" has got ISO3 codes for Western Sahara and South Sudan wrong.
