...
ind = which(allBOTH.filter$PI !='NOx/NOy' & allBOTH.filter$PI !='PAN/NOy' & allBOTH.filter$PI != 'MAtoF' & 
              allBOTH.filter$USEME != -1 & is.finite(allBOTH.filter$USEME) & allBOTH.filter$USEME != 0)
...
for (i in 1:length(varsDIFFS$vars)){
  ...
  if (length(ind) > minD & length(ind2) > minD ){
  ...
  if (length(ind4) > minD & length(ind2) > minD ){
  ...
  if (length(ind) > minD & length(ind5) > minD ){
  ...
  if (length(ind4) > minD & length(ind5) > minD ){
...
# Ok, we don't want to correct monoterpenes for MCE, too much difference across ag fuels
ind = which(varsDIFFS$names == 'Monoterpenes')
varsDIFFS$AvgAgCorr[ind] = NaN
...
# ***** # ***** Ok, need to make consistency now between Table 4, where averages are weighted, 
#and here. So replace ratios with Table 4 ratios but only for species not corrected for MCE.# ***** # ***** 
...
uPLOT2=ggplot(test, aes(x=NUM,y=TableRatio, label=names)) + geom_point(aes(shape=kind, col=kind), size=5) + geom_hline(aes(yintercept=1), lty=2)+
  ...
  ylab('Ratio of Cropland residue EF to Prescribed EF') +
